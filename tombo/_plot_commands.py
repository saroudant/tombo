...
try:
    ...
    _R_DF = r.DataFrame({}) # SM: change here to avoid errors
except:
...
def plot_single_read(
    ...
    # Return before plotting, change by SM
    return {
        'sigDat': sigDat,
        'vlineDat': vlineDat,
        'hDat': hDat,
...
def plot_genome_locations(
    ...
    print(fast5s_dirs, corr_grp, bc_subgrps, pdf_fn,
        ctrl_fast5s_dirs, num_bases, overplot_thresh, overplot_type,
        genome_locs, tb_model_fn, alt_model_fn, plot_default_stnd,
        plot_default_alt)
    if VERBOSE: th.status_message('Parsing genome locations.')
...
# Added by SM for retrieving info
def return_genome_locations_df(
        fast5s_dirs, corr_grp, bc_subgrps, pdf_fn,
        ctrl_fast5s_dirs, num_bases, overplot_thresh, overplot_type,
        genome_locs, tb_model_fn, alt_model_fn, plot_default_stnd,
        plot_default_alt):
    ...
    if ctrl_fast5s_dirs is None:
        if std_ref is None:
            return {
                ...
                'overplot_thresh': overplot_thresh,
                'overplot_type': overplot_type
            }
        else:
            return {
                'plot_intervals': plot_intervals,
